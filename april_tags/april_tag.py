...
class AprilTag:
    ...
    def detect_tags(self, color_image):
        ...
        tags = [tag for tag in tags if tag.decision_margin >
                30 and tag.tag_id in self.tag_poses.keys()]
        return tags
    ...
    def localize(self, tags):
        ...
        best_tag = tags[0]
        for tag in tags:
            if tag.decision_margin > best_tag.decision_margin:
                best_tag = tag

        T_camera_apriltag = self.Rt2T(best_tag.pose_R, best_tag.pose_t)
        T_apriltag_camera = np.linalg.inv(T_camera_apriltag)
        T_map_apriltag = self.tag_poses[best_tag.tag_id]
